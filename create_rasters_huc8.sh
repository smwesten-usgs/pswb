...
/usr/bin/ogr2ogr -sql "SELECT * from $BASENAME WHERE HUC8="\'"$POLYNAME"\' $LOCAL_SHP $MASK_SHPFILE
wait
...
/usr/bin/gdalwarp -r near -overwrite -dstnodata -9999. -te $EXTENTS -tr $RES $RES -csql "SELECT * from $BASENAME WHERE HUC8="\'"$POLYNAME"\' "$TEMPTIF1" "$TEMPTIF2"
...
/usr/bin/gdalwarp -r near -overwrite -dstnodata -9999. -tr $RES $RES -csql "SELECT * from $BASENAME WHERE HUC8="\'"$POLYNAME"\' $TEMPTIF1 $TEMPTIF2
...
/usr/bin/gdalwarp -r mode -overwrite -ot Int32 -of HFA -srcnodata 0 -dstnodata -9999 -te $EXTENTS -tr $RES $RES  -cutline $MASK_SHPFILE -csql "SELECT * from $BASENAME WHERE HUC8="\'"$POLYNAME"\' -crop_to_cutline  "$IMGFILE" $TEMPIMG
/usr/bin/gdal_translate -ot Int32 -a_nodata -9999 -of AAIGrid $TEMPIMG "$OUTPUT_NLCD_NAME" 

/usr/bin/gdalwarp -ot Int32 -r near -s_srs "$VRT_PROJ4" -t_srs "$PROJ4" -te $EXTENTS -overwrite -csql "SELECT * from $BASENAME WHERE HUC8="\'"$POLYNAME"\' $FQ_VRTFILE $TEMPTIF1
...
echo "script: $OUTPUT_NLCD_NAME $OUTPUT_AWC_NAME $OUTPUT_SOILS_NAME $OUTPUT_D8_FLOWDIR_NAME $OUTPUT_SWB_CTL_NAME $LLX $LLY $NX $NY $RES $START_DATE $END_DATE" > craptastic.txt
./write_swbfile.sh $OUTPUT_NLCD_NAME $OUTPUT_AWC_NAME $OUTPUT_SOILS_NAME $OUTPUT_D8_FLOWDIR_NAME $OUTPUT_SWB_CTL_NAME $LLX $LLY $NX $NY $RES $START_DATE $END_DATE
